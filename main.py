...
class Route(db.Model):
    ...
    route_points = db.Column(db.Text)  # JSON string
    route_comments = db.Column(db.Text)  # JSON string
...
def login():
    ...
    if user.current_token and user.current_device != device_id:
        return jsonify({"error": "already logged in"}), 403
...
def send_message():
    ...
    text = data.get("text", "")
    audio = save_uploaded_file(request.files["audio"], "wav") if "audio" in request.files else data.get("audio")
    photo = save_uploaded_file(request.files["photo"], "jpg") if "photo" in request.files else data.get("photo")
    msg = Message(group_id=group_id, sender=sender, receiver=receiver, text=text, audio=audio, photo=photo)
    db.session.add(msg)
    db.session.commit()
...
def get_messages():
    ...
    msgs = Message.query.filter_by(group_id=group_id).order_by(Message.created_at).all()
    return jsonify([{ "id": m.id, "sender": m.sender, "receiver": m.receiver, "text": m.text,
                      "audio": m.audio, "photo": m.photo, "created_at": m.created_at.isoformat() } for m in msgs])
...
def get_routes():
    ...
    return jsonify([{ "id": r.id, "username": r.username, "route_name": r.route_name,
                      "route_points": r.route_points, "route_comments": r.route_comments,
                      "distance": r.distance, "created_at": r.created_at.isoformat() } for r in routes])

# --- SOS ---

@app.route("/sos_report", methods=["POST"])
@jwt_required()
@single_device_required
def sos_report():
    data = request.json
    photo = data.get("photo")
    filename = save_uploaded_file(request.files["photo"], "jpg") if "photo" in request.files else photo
    sos = SOS(username=get_jwt_identity(), lat=data.get("lat"), lon=data.get("lon"),
              text=data.get("text", ""), photo=filename)
    db.session.add(sos)
    db.session.commit()
    return jsonify({"message": "sos saved"})
...
def get_sos():
    ...
    SOS.query.filter(SOS.created_at < cutoff).delete()
    db.session.commit()
    results = SOS.query.order_by(SOS.created_at.desc()).all()
    return jsonify([{ "id": s.id, "username": s.username, "lat": s.lat, "lon": s.lon,
                      "text": s.text, "photo": s.photo,
                      "created_at": s.created_at.isoformat() } for s in results])
