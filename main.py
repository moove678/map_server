...
class Sos(db.Model):
    ...
    # --- SOS_EXT ---
    status = db.Column(db.String(20), default="средняя")  # срочность
    is_danger = db.Column(db.Boolean, default=False)      # опасность для жизни
    photos = db.Column(db.String(500))                    # имена файлов через запятую
    active = db.Column(db.Boolean, default=True)
    rescuer = db.Column(db.String(80), nullable=True)
...
class SosReport(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    sos_id = db.Column(db.Integer, db.ForeignKey("sos.id"))
...
def sos_to_json(s, show_reports=False):
    photos = (s.photos or "").split(",") if s.photos else []
    return {
        "id": s.id,
        "user": s.username,
        "lat": s.lat,
        "lon": s.lon,
        "comment": s.comment,
        "status": getattr(s, "status", "средняя"),
        "is_danger": getattr(s, "is_danger", False),
        "photos": [f"/uploads/{fn}" for fn in photos if fn],
        "active": getattr(s, "active", True),
        "rescuer": getattr(s, "rescuer", None),
        ...
        "reports": [
            ...
            for r in SosReport.query.filter_by(sos_id=s.id).all()
...
def sync():
    ...
    new_sos = [sos_to_json(s) for s in soses]
...
def sos():
    ...
    if request.content_type and request.content_type.startswith("multipart"):
        ...
        status = f.get("status", "средняя")
        is_danger = f.get("is_danger", "false").lower() == "true"
        # До 3-х файлов: photo1, photo2, photo3
        for key in ["photo1", "photo2", "photo3"]:
            if key in request.files:
                fn = save_uploaded_file(request.files[key], "jpg")
                files.append(fn)
    else:
        d = request.json or {}
        lat = d.get("lat", 0.0)
        ...
        status = d.get("status", "средняя")
        is_danger = bool(d.get("is_danger", False))
        if d.get("photos"):
            files = d["photos"] if isinstance(d["photos"], list) else [d["photos"]]
    entry = Sos(
...
@app.route("/report_sos", methods=["POST"])
@jwt_required()
def report_sos():
    d = request.json
    sos_id = d.get("sos_id")
    ...
    if reporter == sos.username:
        return jsonify(error="self_report"), 400
    already = SosReport.query.filter_by(sos_id=sos_id, reporter=reporter).first()
    if already:
        return jsonify(error="already_reported"), 400
    db.session.add(SosReport(sos_id=sos_id, reporter=reporter, comment=comment))
    db.session.commit()
    # Автоскрытие после 3 жалоб
    if SosReport.query.filter_by(sos_id=sos_id).count() >= 3:
        sos.active = False
        db.session.commit()
    return jsonify(success=True)

@app.route("/delete_sos", methods=["POST"])
@jwt_required()
def delete_sos():
    sos_id = request.json.get("sos_id")
    sos = Sos.query.get(sos_id)
...
def resolve_sos():
    sos_id = request.json.get("sos_id")
    sos = Sos.query.get(sos_id)
    helper = get_jwt_identity()
    if not sos or not sos.active or sos.closed:
        return jsonify(error="not_found"), 404
    if helper == sos.username:
        return jsonify(error="self_help"), 400
    sos.rescuer = helper
    sos.closed = True
    sos.active = False
    db.session.commit()
    rescuer = User.query.get(helper)
    if rescuer:
        rescuer.rating = (rescuer.rating or 0) + 1
        db.session.commit()
    return jsonify(success=True)
